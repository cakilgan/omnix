...
namespace engine {
    namespace logger {
        ...
        constexpr log_field_key message_key = {
            ox::hash<ox::cstr>{}("msg"),"msg"
        };
        ...
        template<typename T>
        struct is_log_kv : std::false_type {};

        template<typename T>
        struct is_log_kv<log_kv<T>> : std::true_type {};
        ...
        inline ox::cstr level_to_str(enum log_event::level lvl) {
            switch (lvl) {
                case log_event::level::info: return "info";
        ...
        void fill_fields(log_event& ev, int idx,
                         log_kv<T> kv,
                         Rest... rest)
        {
            if (idx >= 8) return;
        ...
        inline level_style get_level_style(enum log_event::level lvl) {
            switch (lvl) {
                case log_event::level::info:
                    return { ansi::black,        ansi::bg_bright_green,   " INFO      ", ansi::bright_green   };
                case log_event::level::debug:
                    return { ansi::black,        ansi::bg_bright_cyan,    " DEBUG     ", ansi::bright_cyan    };
                ...
                case log_event::level::notice:
                    return { ansi::bright_white, ansi::bg_bright_blue,    " NOTICE    ", ansi::bright_blue    };
                case log_event::level::lifecycle:
                    return { ansi::black,        ansi::bg_bright_magenta, " LIFECYCLE ", ansi::bright_magenta };
                default:
        ...
        inline void process(const log_event& ev) {
            ...
            printf("[%2i.%2i.%2i..%3ius] ",
                ...
                dt.value.minute,
                dt.detail.second,
                dt.detail.microsecond);


            printf("%s%s%s%s ",
                ansi::bold, style.badge_bg, style.badge_fg,
                style.label);
            printf("%s", ansi::reset);

            bool plain_message =
                ev.field_count > 0 &&
                ev.fields[0].key.hash == message_key.hash;

            if (plain_message) {
                printf("%s%s%s",
                    ansi::bold, style.msg_color, ev.fields[0].s);
            ...
            for (ox::u32 i = plain_message ? 1u : 0u; i < ev.field_count; ++i) {
                ...
                printf("  %s%s%s%s = %s",
                    ansi::dim, style.msg_color, f.key.name, ansi::reset,
                    ansi::reset);
