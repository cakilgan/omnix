...
namespace ox {
    namespace containers {
        ...
        private:
            ...
            result_t _u_grow() {
                ...
                T* new_data = static_cast<T*>(
                    _allocator->grow(
                        _data,
                        _u_convert(_capacity),
                        _u_convert(new_capacity)
                    )
                );

                OX_ASSERT(new_data);
                if (!new_data) return memory::err::allocation_error;
        ...
        public:
            ...
            explicit vector(engine::memory::type memory_type = engine::memory::GENERAL, usize reserve = 4)
                :vector(ox::safe(engine::memory::get_allocator<T>(memory_type)),reserve){
            }
            explicit vector(std::initializer_list<T> list,engine::memory::type memory_type = engine::memory::GENERAL, usize reserve = 4)
