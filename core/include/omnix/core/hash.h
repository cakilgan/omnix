...
template <typename T> struct hash {
    usize operator()(const T &value) const { return value.hash(); }
};
...
template <> struct hash<str> {
    usize operator()(str v) const { return hash<strview>{}(v.data()); }
};
