...
struct hashmap {
    ...
    struct bucket {
        ...
        const U &value() const { return _value; }
  ...
  private:
    ...
    void _u_if_grow() {
        ...
        if (static_cast<f32>(_occupied_buckets) / _bucket_count > MAX_LOAD)
            _u_grow();
    }

    template <typename K, typename V> void _u_put(K &&key, V &&value) {
        _u_if_grow();
        _u_put_no_grow(ox::forward<K>(key), ox::forward<V>(value));
    }
    template <typename K, typename V> void _u_put_no_grow(K &&key, V &&value) {

        usize idx =
            hash</*warn: dont change it ->*/ T>{}(key) & (_bucket_count - 1);
  ...
  public:
    explicit hashmap(Allocator *allocator, const usize reserve_bucket = 4)
        ...
        _buckets = static_cast<bucket *>(
            allocator->alloc(_u_convert_bucket(_bucket_count)));
        for (usize i = 0; i < _bucket_count; ++i)
            new (&_buckets[i]) bucket{};
    }
    ...
    hashmap(hashmap &&other) noexcept
        : _allocator(other._allocator), _bucket_count(other._bucket_count),
          _buckets(other._buckets), _occupied_buckets(other._occupied_buckets) {
    ...
    struct iterator {
        ...
        iterator(bucket *start, bucket *end) : _current(start), _end(end) {
            while (_current < _end && _current->_state != occupied)
                ++_current;
        }
    ...
    struct const_iterator {
        ...
        const_iterator(const bucket *start, const bucket *end)
    ...
    iterator end() {
        return iterator(_buckets + _bucket_count, _buckets + _bucket_count);
    }
    const_iterator begin() const {
        return const_iterator(_buckets, _buckets + _bucket_count);
    }
    const_iterator end() const {
        return const_iterator(_buckets + _bucket_count,
