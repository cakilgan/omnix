...
struct freelist_allocator : allocator {
  private:
    void coalesce() {
        ...
        while (b && b->next) {
            byte *expected =
                reinterpret_cast<byte *>(b) + (size_of<block>() + b->size);
  ...
  public:
    explicit freelist_allocator(memory get_memory)
        : allocator(ox::move(get_memory)) {
            ...
            if (b->size >= aligned_size) {
                if (b->size >= aligned_size + size_of<block>()) {
                    ...
                    const auto new_block = reinterpret_cast<block *>(
                        reinterpret_cast<byte *>(b) +
                        (size_of<block>() + aligned_size));
    ...
    result_t free(vptr ptr) {
        ...
        if (static_cast<byte *>(ptr) < static_cast<byte *>(_mem.data()) ||
            static_cast<byte *>(ptr) >=
                static_cast<byte *>(_mem.data()) + _mem.size())
            return results::err::invalid_parameter;

        const auto b = reinterpret_cast<block *>(static_cast<byte *>(ptr) -
                                                 size_of<block>());
        block *prev = nullptr;
    ...
    vptr grow(vptr ptr, bytes old_size, bytes new_size) {
        ...
        auto *in_place = reinterpret_cast<block *>(static_cast<byte *>(ptr) -
                                                   size_of<block>());
            ...
            if (total >= aligned_new) {
                ...
                if (remaining >= size_of<block>()) {
                    auto *new_block = reinterpret_cast<block *>(
                        reinterpret_cast<byte *>(in_place) + size_of<block>() +
                        aligned_new);

                    new_block->size = remaining - size_of<block>();
                    block *fp = nullptr;
                    block *fc = _free_list;
                    while (fc && fc < new_block) {
                        fp = fc;
                        fc = fc->next;
                    }
                    new_block->next = fc;
    ...
    result<bytes> size(vptr ptr) const {
        ...
        if (static_cast<byte *>(ptr) < static_cast<byte *>(_mem.data()) ||
            static_cast<byte *>(ptr) >=
                static_cast<byte *>(_mem.data()) + _mem.size())
            return results::err::invalid_parameter;
        auto b = reinterpret_cast<block *>(static_cast<byte *>(ptr) -
                                           size_of<block>());
        OX_ASSERT(b);
        return b->size;
    }
